...
0 0 0 1 1
0 1 1 1 0
0 0 1 0 0  //tricky input
is the input  

Some doubt in creation of newnode for the initial status of  parent all getting to NULL*/
...
void solve(int initial[N][N])
{
    ...
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<N;j++)
        {
                temp=newnode(initial,0,i,j,NULL);//some doubt if one doesnt lead to solution..
                temp->cost = claculateCost(temp->mat);
               // cout<<"level = "<<temp->level<<" index of tapping "<<i<<" "<<j<<"cost = "<<temp->cost<<endl;
